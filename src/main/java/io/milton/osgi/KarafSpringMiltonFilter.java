...
/**
 * Adapt SpringMiltonFilter to make it runnable on Karaf.
 * Due to the context variable having private scope it was not possible to simply extend SpringMiltonFilter
 * 
 * @author fgiloux
 *
 */
...
* @author bradm
*/
public class KarafSpringMiltonFilter implements javax.servlet.Filter {
	 ...
	 */
	protected String[] excludeMiltonPaths;
	...
	/* 
	 * getRealPath() is not supported in Karaf and getContextPath() must be used instead:
	 * File webRoot = new File(fc.getServletContext().getRealPath("/")); ->
	 * File webRoot = new File(fc.getServletContext().getContextPath()); 
	 */
	...
	protected void initSpringApplicationContext(FilterConfig fc) {
		...
		log.info("Context path: " + fc.getServletContext().getContextPath());
		File webRoot = new File(fc.getServletContext().getContextPath());
